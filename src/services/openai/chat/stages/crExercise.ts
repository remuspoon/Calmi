...
const CR_EXERCISE: RESPONSE_TYPE | RESPONSE_TYPE[] = [
    {response: (messages) => chatCompletions(messages, "Based on the user's automatic thoughts, ask the user one question that challenges their cognitive distortion.")},

    {response: (messages) => chatCompletions(messages, "Respond to the user in less than two sentences. Afterwards, build upon the argument by asking another question that challenges the cognitive distortion.")},
...
{response: async (messages) => {
    ...
    const res = staticResponse([gptResponse, 'Based on your answers above, I want you to create a new thought or an alternative response to your situation that allows you to move forward and achieve your ideal outcome.',
