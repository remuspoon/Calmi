...
const RAPPORT_BUILDING: RESPONSE_TYPE | RESPONSE_TYPE[] = [
  {response: (messages) => chatCompletions(messages, "In 4 sentences or less, Empathize with the user's feelings and situation without repeating it word for word. Do not offer solutions. Afterwards, ask one question to gather more information on the user's situation.")},

  {response: (messages) => chatCompletions(messages, "In 2 sentences, ask one question to gather more information on the user's situation. Do not repeat questions asked before.")},
  ...
  {response: staticResponse("Great! In this exercise, we are going to identify your distorted 'Automatic thoughts'; and rethink them into something more constructive. Do you know what is an Automatic Thought in CBT?"),
    next: async (messages) => {
      const params = lastbotanduser(messages as any)
      const userAffirm = await userAffirmed(params)
