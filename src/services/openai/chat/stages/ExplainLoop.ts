...
const EXPLAIN_LOOP: RESPONSE_TYPE | RESPONSE_TYPE[] = [
  {
    response: (messages) =>
      chatCompletions(
        messages,
        "In one sentence, ask the user what they don't understand."
      )
  },

  {
    response: (messages) =>
      chatCompletions(
        messages,
        'In 4 sentences, explain to the user based on their response. Finish by asking them if they understand what an automatic thought is.'
      ),
